...
def test_calculate_substat_roll_value(substat: SubStat) -> None:
    ...
    roll_value = analysis.calculate_substat_roll_value(substat)
...
def test_calculate_substat_rolls(substat: SubStat, artifact: Artifact) -> None:
    ...
    assert analysis.calculate_substat_rolls(substat) == 2

    expected_rolls = (1, 1, 1, 2)
    for idx, sub in enumerate(artifact.substats):
        assert analysis.calculate_substat_rolls(sub) == expected_rolls[idx]
...
def test_calculate_artifact_maximum_roll_value(artifact: Artifact) -> None:
    ...
    maximum_roll_value = analysis.calculate_artifact_maximum_roll_value(artifact)
...
def test_create_random_artifact() -> None:
    ...
    artifact_a = analysis.create_random_artifact(ArtifactSlot.SANDS)
    assert artifact_a.artifact_slot == ArtifactSlot.SANDS
    assert artifact_a.rarity == 5

    artifact_b = analysis.create_random_artifact(ArtifactSlot.PLUME, 4)
...
def test_upgrade_artifact_to_max(artifact: Artifact) -> None:
    ...
    artifact = analysis.upgrade_artifact_to_max(artifact)
...
def test_create_multiple_random_artifacts() -> None:
    ...
    artifacts = analysis.create_multiple_random_artifacts(5)
